...
| FR-059 Dejar fuera la animación | `core/domain/genres.ts`, `core/domain/filters.ts`, `core/agent/top-year.ts`, `core/providers/tmdb.ts` (`without_genres`), `renderer/views/SettingsView.tsx` | `unit/genres.test.ts` (7 casos, incluido el falso positivo de «Reanimación»), `unit/filters.test.ts` → «exclusión de la animación», `unit/top-year.test.ts` → exclusión por identificador y red de seguridad | Automática |
| FR-058 Las mejores de un año | `core/domain/ranking.ts`, `core/domain/ranking-format.ts`, `core/agent/top-year.ts`, `core/providers/tmdb.ts` (`topRated`), `renderer/views/Rankings.tsx`, `scripts/agent-cli.ts` (`--ranking`) | `unit/ranking.test.ts` (18 casos, incluido el que fija que esta media **no** es el índice ponderado) y `unit/top-year.test.ts` (4 casos, camino completo con proveedores simulados) | Automática la lógica y el formato; los datos reales dependen de la red y de las claves del usuario |
...
| FR-056 (las direcciones) | Los buscadores de las plataformas no se pueden abrir desde aquí: el inventario de red está cerrado a cuatro anfitriones (ADR-010) y ninguna lo es. Se comprueba la **forma** del enlace, no que responda | T100 |
